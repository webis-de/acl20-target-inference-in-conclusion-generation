...
class FunctionNegativeTripletSelectorForTargetEmbedding(TripletSelector):
    """
    For each positive pair, takes the hardest negative sample (with the greatest triplet loss value) to create a triplet
    Margin should match the margin used in triplet loss.
    negative_selection_fn should take array of loss_values for a given anchor-positive pair and all negative samples
    ...
    def get_triplets(self, conc_embeddings, p_embeddings, labels):
        if self.cpu:
            embeddings = embeddings.cpu()
        distance_matrix = pdist(embeddings)
        distance_matrix = distance_matrix.cpu()
...
def save_checkpoint(model_dir, step, states, keep_max=None):
    filepath = model_dir + "{}-{}.pt".format(_checkpoint_prefix, step)
